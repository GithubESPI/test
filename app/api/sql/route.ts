...
export async function POST(request: Request) {
  try {
    ...
    const queries: Record<keyof QueryResults, string> = {
      ...
      // Exemple pour MATIERE (appliquer la même logique à MOYENNES_UE)
      MATIERE: `
    ...
    ORDER BY ap.NOM_APPRENANT, ap.PRENOM_APPRENANT
      ...
      NOTES: `
        ...
        WHERE r.CODE_SESSION = 5 
          AND a.CODE_APPRENANT IN (SELECT i.CODE_APPRENANT FROM INSCRIPTION i INNER JOIN FREQUENTE f ON i.CODE_INSCRIPTION = f.CODE_INSCRIPTION WHERE f.CODE_GROUPE = ${group})
      `,
    };
